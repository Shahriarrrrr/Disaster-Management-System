...
export default function DonationList({ donation, user, searchTerm = "" }) {
  ...
  const filteredDonations = useMemo(() => {
    return donation.filter((d) => {
      ...
      if (filterCampaign && d.donation_cause_display !== filterCampaign)
        return false;
      if (filterMinAmount && d.donation_amount < Number(filterMinAmount))
        return false;
      if (filterStartDate && donationDate < new Date(filterStartDate))
        return false;
  ...
  const currentDonations = reversedDonations.slice(
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {formatCurrency(user_total_donate)}
              </p>
            </div>
          </div>
        </div>
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {filteredDonations.length > 0
                  ? formatCurrency(user_total_donate / filteredDonations.length)
                  : "$0"}
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div className="flex items-center space-x-4">
              <div className="text-sm text-gray-400">
                Showing{" "}
                {filteredDonations.length === 0 ? 0 : indexOfFirstDonation + 1}{" "}
                to {Math.min(indexOfLastDonation, filteredDonations.length)} of{" "}
